...
local function emit_info()
    awful.spawn.easy_async_with_shell("sh -c 'spotify_title'",
        function(stdout)
            local artist = stdout:match('^.-|'):sub(1,-2)
            local title = stdout:match('%|.*%|'):sub(3,-1):sub(1,-3)
            local status = stdout:sub(-2,-1)
